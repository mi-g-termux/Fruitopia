...
setProducts([...products]);
  ...
  const addCoupon    = async (coupon: Coupon)   => { await dbService.saveCoupon(coupon);    setCoupons(prev => [...prev, coupon]); };
   ...
   *
   * C1 LOGIN: After local credentials pass (called with loggedIn=true), we
   * fire a best-effort Firebase Auth sign-in using a synthetic email address
   * derived from the admin username.  If Firebase Auth fails for any reason
   * (not configured, wrong password, network error) we only warn — the local
   * session is still granted so the admin panel is never blocked.
   *
   * C2 LOGOUT: Before clearing the local session (loggedIn=false) we call
   * fbSignOut(auth) so the Firebase Auth token is invalidated server-side.
   * Errors are swallowed silently.
   *
   * The caller is responsible for passing `username` and `password` when
   * loggedIn=true so that the Firebase Auth call can derive the synthetic
   * admin email.  When loggedIn=false those parameters are unused.
   */
      ...
      if (username && password && auth) {
        ...
        (async () => {
          try {
            ...
            console.warn('[Auth] Firebase Auth sign-in failed:', e);
            // Do NOT block login — just warn. Local credentials still work.
          }
        })();
      }
    } else {
